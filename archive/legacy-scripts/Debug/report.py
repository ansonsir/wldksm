...
class Report:
    def __init__(self, scan_datas, result_file, template_file, output_file, redArea_file):
        self.template_file = str(Path(__file__).parent / template_file)
        self.output_file = output_file
        self.result_file = result_file
        self.scan_datas = scan_datas
        self.redArea_file = redArea_file
        self.redArea_list = []
        self.inRedArea_list = []
        self.ip_ports_dict = {}
    ...
    def load_redArea(self):
        with open(self.redArea_file, 'r') as f:
            for line in f:
                line = line.strip()
                ...
                try:
                    ...
                    self.redArea_list.append(network)
                except ValueError as e:
                    print(f"无效网段格式: {line} -> 错误: {e}")
    ...
    def Analysis_data(self):
        for ip, ports in self.ip_ports_dict.items():
            ...
            ip_judge = ip_address(ip)
            if any(ip_judge in network for network in self.redArea_list):
                self.inRedArea_list.append(f'{ip} {ports}')
        ...
        if len(self.inRedArea_list) == 0:
            self.template_fields['p_redArea_devices_num'] = "没有设备"
            self.template_fields['p_redArea_devices_info'] = '没有设备\n'
        else:
            self.template_fields['p_redArea_devices_num'] =  f"共检测到{len(self.inRedArea_list)}台设备"
            self.template_fields['p_redArea_devices_info'] = '\n'.join(self.inRedArea_list)
